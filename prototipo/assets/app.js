...
(function () {
  ...
  var TEMAS = ["Direito Penal", "Constitucional", "Direito Civil", "Direito Administrativo"];
  var DIFICULDADES = [
  ...
  var TEMA_INFO = {
    "Direito Penal": {
      color: "#a53e00",
      iconInner: '<g transform="rotate(-45 12 12)"><rect x="7.5" y="4" width="9" height="5.5" rx="1.6"/><line x1="12" y1="9.5" x2="12" y2="16.5"/></g><line x1="6" y1="20" x2="18" y2="20"/>'
    },
    "Constitucional": {
      color: "#724aab",
      iconInner: '<path d="M12 4.5c-1.8-1-4.3-1.4-6-1v13.5c1.7-.4 4.2 0 6 1 1.8-1 4.3-1.4 6-1V3.5c-1.7-.4-4.2 0-6 1z"/><line x1="12" y1="4.5" x2="12" y2="18"/>'
    },
    "Direito Civil": {
      color: "#007a5f",
      iconInner: '<path d="M6 3h8l4 4v14H6z"/><path d="M14 3v4h4"/><line x1="9" y1="12" x2="15" y2="12"/><line x1="9" y1="15.5" x2="15" y2="15.5"/>'
    },
    "Direito Administrativo": {
      color: "#33607e",
      iconInner: '<rect x="3.5" y="8.5" width="17" height="11" rx="1.8"/><path d="M8.5 8.5V6.3A2.3 2.3 0 0 1 10.8 4h2.4a2.3 2.3 0 0 1 2.3 2.3v2.2"/><line x1="3.5" y1="13.5" x2="20.5" y2="13.5"/>'
    }
  };
  ...
  function buildHome() {
    ...
    TEMAS.forEach(function (t) {
      ...
      b.type = "button";
      b.className = "chip chip--tema" + (t === state.tema ? " is-selected" : "");
      b.setAttribute("data-od-id", "tema-chip-" + slug(t));
      if (t === state.tema && info) {
        b.style.setProperty("--tema-color", info.color);
        b.style.setProperty("--tema-tint", "color-mix(in srgb, " + info.color + " 14%, white)");
      }
    ...
    DIFICULDADES.forEach(function (d) {
      var b = document.createElement("button");
      b.type = "button";
      b.className = "chip chip--dif chip--" + d.value + (d.value === state.dificuldade ? " is-selected" : "");
  ...
  function sortearTemaEDificuldade() {
    ...
    var temaChip = document.querySelector('[data-od-id="tema-chip-' + slug(escolha.tema) + '"]');
    var difChip = document.querySelector('[data-od-id="dificuldade-chip-' + escolha.dificuldade + '"]');
  ...
  function lerMeta() {
    try {
      ...
      if (!raw || raw.data !== hojeStr()) return { data: hojeStr(), respondidas: 0 };
      return raw;
    } catch (e) {
  ...
  function renderMetaDiaria() {
    ...
    if (label) label.textContent = Math.min(m.respondidas, META_ALVO) + " / " + META_ALVO + " hoje";
    if (card) card.classList.toggle("is-complete", m.respondidas >= META_ALVO);
  }

  // ---------- Cronômetro ----------
  function lerCronometroPref() {
    try { return localStorage.getItem(CRONOMETRO_KEY) === "1"; } catch (e) { return false; }
  }
  function salvarCronometroPref(v) {
    try { localStorage.setItem(CRONOMETRO_KEY, v ? "1" : "0"); } catch (e) {}
  }
  ...
  function reagirMascote(tipo) {
    ...
    avatar.classList.add((tipo === "acerto" || tipo === "powerup") ? "is-happy" : "is-sad");
    clearTimeout(mascotTimeout);
    mascotTimeout = setTimeout(function () { bubble.textContent = ""; }, 2200);
  }
  ...
  function concederPowerupSeElegivel() {
    if (state.combo > 0 && state.combo % 3 === 0 && state.powerups.eliminar < POWERUP_MAX) {
  ...
  function renderPowerups() {
    ...
    if (btnPular) btnPular.disabled = state.respondida || state.powerups.pular <= 0;
    if (btnEliminar) btnEliminar.disabled = state.respondida || state.powerups.eliminar <= 0 || state.eliminadasQuestaoAtual;
    if (countPular) countPular.textContent = String(state.powerups.pular);
    if (countEliminar) countEliminar.textContent = String(state.powerups.eliminar);
  }
  ...
  function usarPular() {
    ...
    proxima.textContent = state.indice + 1 < state.rodada.length ? "Próxima questão" : "Ver resultado";
  ...
  function usarEliminar() {
    ...
    if (state.respondida || state.powerups.eliminar <= 0 || state.eliminadasQuestaoAtual || !q) return;
    state.powerups.eliminar--;
    state.eliminadasQuestaoAtual = true;
    var errados = [];
    q.alternativas.forEach(function (_, idx) { if (idx !== q.resposta_correta) errados.push(idx); });
  ...
  function renderQuestao() {
    ...
    $("game-progress").textContent = "Questão " + (state.indice + 1) + " de " + state.rodada.length;
    var temaDot = $("game-tema-dot");
    if (temaDot) temaDot.style.background = (TEMA_INFO[state.tema] && TEMA_INFO[state.tema].color) || "var(--muted)";
    ...
    if (barFill) barFill.style.width = ((state.indice) / state.rodada.length * 100) + "%";
    ...
    q.alternativas.forEach(function (alt, i) {
      ...
      b.appendChild(letra);
      b.appendChild(texto);
      b.addEventListener("click", function () { responder(i, q); });
      wrap.appendChild(b);
    });
    ...
    if (timerWrap) timerWrap.classList.toggle("is-active", state.cronometroAtivo);
  ...
  function finalizarRodada() {
    ...
    var melhorAnterior = historicoAntes
      .filter(function (r) { return r.tema === state.tema && r.dificuldade === state.dificuldade; })
      .reduce(function (max, r) { return Math.max(max, r.aproveitamento); }, -1);
    ...
    if (pct > 70) {
      for (var c = 0; c < 24; c++) {
        ...
        piece.style.background = c % 2 === 0 ? "var(--accent)" : "var(--success)";
  ...
  function renderResultTema() {
    ...
    dot.style.background = (TEMA_INFO[state.tema] && TEMA_INFO[state.tema].color) || "var(--muted)";
    el.appendChild(dot);
    var difLabel = DIFICULDADES.filter(function (d) { return d.value === state.dificuldade; })[0].label;
  ...
  function starSvg(filled) {
    ...
    path.setAttribute("d", "M12 2.5l2.9 6.1 6.6.7-4.9 4.6 1.3 6.6L12 17.3l-5.9 3.2 1.3-6.6-4.9-4.6 6.6-.7L12 2.5z");
  ...
  function renderHistorico() {
    ...
    hist.forEach(function (r) {
      ...
      var dataFmt = d.toLocaleDateString("pt-BR", { day: "2-digit", month: "short" }) + " · " +
        d.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" });
      var corTema = (TEMA_INFO[r.tema] && TEMA_INFO[r.tema].color) || "var(--muted)";

      card.innerHTML =
        '<div class="hist-card-top">' +
          '<span class="hist-tema"><span class="tema-dot" style="background:' + corTema + '"></span>' + r.tema + '</span>' +
          '<span class="hist-dif hist-dif--' + r.dificuldade + '">' + (DIF_LABEL[r.dificuldade] || r.dificuldade) + '</span>' +
        '</div>' +
        '<div class="hist-card-bottom">' +
          '<span class="hist-data">' + dataFmt + '</span>' +
          '<span class="hist-score">' + r.acertos + '/' + r.questoes_total + ' · ' + r.aproveitamento + '%</span>' +
        '</div>' +
        '<div class="hist-bar"><i style="width:' + r.aproveitamento + '%"></i></div>';
  ...
  function init() {
    ...
    if (btnAleatorio) btnAleatorio.addEventListener("click", sortearTemaEDificuldade);
